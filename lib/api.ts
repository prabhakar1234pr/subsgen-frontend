...
export async function processVideo(file: File, style: string, apiUrl = DEFAULT_API_URL): Promise<Blob> {
...
export async function processVideos(files: File[], style: string, apiUrl = DEFAULT_API_URL): Promise<Blob> {
  const fd = new FormData();
  files.forEach(f => fd.append("videos", f)); fd.append("style", style);
  const r = await fetch(`${apiUrl}/api/process-reel`, { method: "POST", body: fd });
  if (!r.ok) { const e = await r.json().catch(() => ({})); throw new Error(e.detail || "Failed"); }
  return r.blob();
}
...
export async function processReelPipeline(files: File[], apiUrl = DEFAULT_API_URL): Promise<ReelResult> {
  const fd = new FormData();
  files.forEach(f => fd.append("videos", f));
  const r = await fetch(`${apiUrl}/api/reel-pipeline`, { method: "POST", body: fd });
  if (!r.ok) { const e = await r.json().catch(() => ({})); throw new Error(e.detail || "Pipeline failed"); }
